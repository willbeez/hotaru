...
function changeImage(){
	imgLink = $('#post_img').val();
        ...
        // to clear picture if user updates image with blank url
        if (!imgLink) {
          $("#cropbox_wrapper").html('');
		...
		images_found.each(function(){
			...
			$(img).load(function(){
				...
				if(width >= <?php echo $h->vars['post_images_settings']['w']; ?> && height >= <?php echo $h->vars['post_images_settings']['h']; ?>){
                                        if (typeof image_array[$(this).attr('src')] != 'undefined') { return; }
                                        // store image src in array and, if we've already displayed it, don't display a duplicate
                                        image_array[($(this).attr('src'))] = 1;
