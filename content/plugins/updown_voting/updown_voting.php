 ...
 * type: vote
 * requires: users 1.1
 * hooks: install_plugin, theme_index_top, post_read_post, header_include, show_post_title, pre_show_post, admin_plugin_settings, admin_sidebar_plugin_settings, post_add_post, submit_confirm_pre_trackback, show_post_extra_fields, show_post_extras, post_delete_post
 * author: Nick Ramsay
...
class UpdownVoting
{
    ...
    public function theme_index_top($h)
    {
        ...
        // Exit if this page name does not contain 'updown_voting'
        if (strpos($h->pageName, 'updown_voting') === false)
        {
            return false;
        }
    ...
    public function doAjax($h)
    {
        // Include UpdownVotingFunctins
        include_once(PLUGINS . 'updown_voting/libs/UpdownVotingFunctions.php');
        $updownFuncs = new UpdownVotingFunctions();
        ...
        if ($h->cage->post->keyExists('post_id')) {
            ...
            $updownFuncs->vote($h, $post_id, $vote_rating, $user_ip, $undo, $updown_voting_settings);
        }
    }
