...
class AkismetFunctions {
	...
	public function __construct($h, $blogURL, $wordPressAPIKey) {
		...
		$this->comment['blog'] = $blogURL;
                ...
                $user_agent = $this->cage->server->getRaw('HTTP_USER_AGENT');
                $referer = $h->cage->server->getRaw('HTTP_REFERER');
                $remote_addr = $h->cage->server->getRaw('REMOTE_ADDR');
	...
	private function getQueryString() {                
		foreach($this->cage->server as $key => $value) {
